...
export const postResolvers = {
    ...
    Post: {
        ...
        comments: (post, {first = 10, offset = 10}, context: ResolverContext, info: GraphQLResolveInfo) => {
    ...
    Query: {
        posts: (parent, {first = 10, offset = 10}, context: ResolverContext, info: GraphQLResolveInfo) => {
        ...
        updatePost: compose(...authResolvers)((parent, { id, input }, {db, authUser}: {db: DbConnection, authUser: AuthUser}, info: GraphQLResolveInfo) => {
            ...
            return db.sequelize.transaction((t: Transaction) => {
              return db.Post
                ...
                .then((post: PostInstance) => {
                  throwError(!post,`Post with id ${id} not found!`);
                  throwError(post.get('author') != authUser.id, `Unauthorize! You can only edit posts by yourself!`);
                  input.author = authUser.id;
        ...
        deletePost: compose(...authResolvers)((parent, { id }, {db, authUser}: {db: DbConnection, authUser: AuthUser}, info: GraphQLResolveInfo) => {
            ...
            return db.sequelize.transaction((t: Transaction) => {
              return db.Post
                ...
                .then((post: PostInstance) => {
                  throwError(!post,`Post with id ${id} not found!`);
                  throwError(post.get('author') != authUser.id, `Unauthorize! You can only delete posts by yourself!`);
